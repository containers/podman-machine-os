...
var _ = Describe("run image tests", Ordered, ContinueOnFailure, func() {
	...
	Context("when the podman machine is up and running", func() {
		...
		It("should load Rosetta activation service", func() {
			skipIfNotVmtype(define.AppleHvVirt, "Rosetta service is activated when the provider is applehv")
		...
		It("should have zero critical error messages journalctl", func() {
			skipIfVmtype(define.LibKrun, "TODO: analyze the error messages in journalctl when using libkrun")
			skipIfVmtype(define.AppleHvVirt, "TODO: analyze the error messages in journalctl when using applehv")
		...
		It("should start gvforwarder services successfully on windows", func() {
			skipIfNotVmtype(define.HyperVVirt, "gvforwarder is started on Windows only")
